/**
 * 
 * measures weight with 10, 20, ..80 measurements
 *
 */
...
//  Serial.println("---\t\t---\t\t---\t\t---\t\t---\t\t---\t\t---\t\t---");
  ...
  {
    wReadings += measureWeight();
    thisPayload.w[i] = (wReadings/(i+1));
    //Serial.print(wReadings);Serial.print("] ");
    Serial.print((thisPayload.w[i]/10000.0),4);
    Serial.print(", ");

  }
    scale.power_up(); //powers on HX711
    scale.set_scale(calibration_factor[0]); //sets calibration to this LC
    scale.set_offset(zero_factor[0]);       //sets zero to this LC
    //scale.get_units(10);
    Blink(20,scaleNmeasurements);
    wReadings = round(scale.get_units(80)*10000);
    Serial.print((wReadings/10000.0),4);
    scale.power_down();
  Serial.println();
...
double measureWeight()
{
    ...
    //scale.get_units(10);
    Blink(20,scaleNmeasurements);
    wtemp = round(scale.get_units(scaleNmeasurements)*10000);
